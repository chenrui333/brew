...
module Cask
  ...
  class URL
    # Reached via `def_delegators :uri, ...` (Forwardable) and the audit checks,
    # so it has no static callers for `brew deadcode` to find.
    # deadcode:keep
    ...
    def initialize(
      ...
      specs[:revisions]  = T.let(revisions, T.nilable(T::Hash[T.any(Symbol, String), String]))
      specs[:revision]   = @revision = T.let(revision, T.nilable(String))
      specs[:trust_cert] = T.let(trust_cert, T.nilable(T::Boolean))
      specs[:cookies]    =
      ...
      specs[:data]       = T.let(data, T.nilable(T::Hash[String, String]))
